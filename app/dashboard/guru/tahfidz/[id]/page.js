...
export default function PesertaDetailPage() {
  ...
  const filteredBySearch = perkembangan.filter((row) =>
    row.surah.toLowerCase().includes(search.toLowerCase())
  );
  ...
  const exportPDF = () => {
    ...
    doc.setFontSize(16);
    doc.text(
      `Laporan Perkembangan Tahfidz - ${data.nama_siswa}`,
      margin.left,
      margin.top
    );
    ...
    autoTable(doc, {
      head: [["Tanggal", "Surah", "Ayat", "Catatan"]],
      body: rows,
    ...
    doc.save(`perkembangan_${data.nama_siswa}.pdf`);
  };
  ...
  const handleSubmit = async () => {
    ...
    if (res.ok) {
      ...
      setOpenModal(false);
      setEditData(null);
      setForm({ tanggal: "", surah: "", ayat: "", catatan: "" });
    }
  };
                ...
                >
                  <div className="flex justify-between items-center bg-gray-100 px-4 py-2 text-sm">
                    <div>
                      ...
                      {" | "}
                      <span className="font-medium">Surah:</span> {row.surah}{" "}
                      <span className="font-medium">Ayat:</span> {row.ayat}
                    </div>
                    <div className="flex gap-2">
                      ...
                      <Button
                        size="icon"
                        variant="destructive"
                        onClick={() => handleDelete(row.id)} //id perkembangan_tahfidz
                      >
        ...
        <DialogContent>
          ...
          <div className="space-y-3">
            ...
            <Input
              placeholder="Ayat"
              value={form.ayat}
              onChange={(e) => setForm({ ...form, ayat: e.target.value })}
            />
            <Input
              placeholder="Catatan"
              value={form.catatan}
