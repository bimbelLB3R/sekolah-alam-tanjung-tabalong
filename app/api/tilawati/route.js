...
export async function GET() {
  try {
    ...
    const [rows] = await pool.execute(
      "SELECT id,created_at, nama_siswa, nama_rombel,pembimbing FROM peserta_tilawati ORDER BY created_at DESC"
    );
...
export async function POST(request) {
  ...
  `INSERT INTO peserta_tilawati (id, nama_siswa, nama_rombel, pembimbing) 
   VALUES (UUID(), ?, ?, ?)`,
