...
"use client";
import { useState } from "react";
import { useRouter, useSearchParams } from "next/navigation"; // ✅ DITAMBAHKAN
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    const res = await fetch("/api/auth/login", {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ email, password }),
      cache: "no-store", // ✅ PENTING: agar cookie terbaru langsung terbaca
    });
    ...
    if (res.ok) {
      setMessage(`✅ ${data.message}, Selamat datang ${data.user.name}`);
      setLoading(false)
      // ✅ AMBIL CALLBACK URL DARI MIDDLEWARE
      const callbackUrl = searchParams.get("callbackUrl");
      const safeRedirect = callbackUrl?.startsWith("/") ? callbackUrl : "/dashboard";
   
      // arahkan ke dashboard setelah 1 detik
      setTimeout(() => {
        router.replace(safeRedirect); // ✅ replace agar tidak bisa back ke login
      }, 1000);
    } else {
      setMessage(`❌ ${data.error}`);
    }
