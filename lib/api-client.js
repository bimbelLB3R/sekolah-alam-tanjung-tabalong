...
/**
 * Event Management API Client
 * Wrapper untuk semua API calls ke backend dengan Role-Based Access Control
 */
...
export const ALLOWED_ROLES = {
  FULL_ACCESS: ['superadmin', 'manajemen'],
  READ_ONLY: ['guru', 'siswa']
};
 ...
 * @param {Object} user - User object from /api/me
 * @param {Array<string>} allowedRoles - Array of allowed role names
 * @returns {boolean}
 */
export function hasPermission(user, allowedRoles = ALLOWED_ROLES.FULL_ACCESS) {
  if (!user || !user.role_name) {
    return false;
  }
  return allowedRoles.includes(user.role_name.toLowerCase());
}

/**
 * Validate user permission before action
...
function validatePermission(user, action) {
  if (!hasPermission(user)) {
    throw new PermissionError(
      `Anda tidak memiliki izin untuk ${action}. Hanya superadmin dan manajemen yang diizinkan.`
    );
  }
}
...
export class EventAPI {
  ...
  /**
   * Create new event (requires permission)
   * @param {Object} data - Event data
  ...
  static async create(data, user) {
    try {
      validatePermission(user, 'membuat event');
  ...
  static async update(id, data, user) {
    try {
      validatePermission(user, 'mengupdate event');
  ...
  static async delete(id, user) {
    try {
      validatePermission(user, 'menghapus event');
  ...
  /**
   * Add committee member (requires permission)
   * @param {number|string} eventId
   * @param {Object} data - Committee member data
   * @param {Object} user - Current user object
   * @returns {Promise<Object>}
   */
  static async addCommitteeMember(eventId, data, user) {
    try {
      validatePermission(user, 'menambah anggota panitia');
  ...
  /**
   * Update committee member (requires permission)
   * @param {number|string} eventId
   * @param {number|string} committeeId
   * @param {Object} data - Updated data
   * @param {Object} user - Current user object
   * @returns {Promise<Object>}
   */
  static async updateCommitteeMember(eventId, committeeId, data, user) {
    try {
      validatePermission(user, 'mengupdate anggota panitia');
  ...
  /**
   * Delete committee member (requires permission)
   * @param {number|string} eventId
   * @param {number|string} committeeId
   * @param {Object} user - Current user object
   * @returns {Promise<Object>}
   */
  static async deleteCommitteeMember(eventId, committeeId, user) {
    try {
      validatePermission(user, 'menghapus anggota panitia');
  ...
  /**
   * Reorder committee members (requires permission)
   * @param {number|string} eventId
   * @param {Array} orders - Array of {id, sort_order}
   * @param {Object} user - Current user object
   * @returns {Promise<Object>}
   */
  static async reorderCommittee(eventId, orders, user) {
    try {
      validatePermission(user, 'mengatur ulang panitia');
  ...
  /**
   * Add todo (requires permission)
   * @param {number|string} eventId
   * @param {Object} data - Todo data
   * @param {Object} user - Current user object
   * @returns {Promise<Object>}
   */
  static async addTodo(eventId, data, user) {
    try {
      validatePermission(user, 'menambah todo');
  ...
  /**
   * Update todo (requires permission)
   * @param {number|string} eventId
   * @param {number|string} todoId
   * @param {Object} data - Updated data
   * @param {Object} user - Current user object
   * @returns {Promise<Object>}
   */
  static async updateTodo(eventId, todoId, data, user) {
    try {
      validatePermission(user, 'mengupdate todo');
  ...
  /**
   * Update todo status (requires permission)
   * @param {number|string} eventId
   * @param {number|string} todoId
   * @param {string} status - pending|in_progress|completed|cancelled
   * @param {Object} user - Current user object
   * @returns {Promise<Object>}
   */
  static async updateTodoStatus(eventId, todoId, status, user) {
    try {
      validatePermission(user, 'mengupdate status todo');
  ...
  /**
   * Delete todo (requires permission)
   * @param {number|string} eventId
   * @param {number|string} todoId
   * @param {Object} user - Current user object
   * @returns {Promise<Object>}
   */
  static async deleteTodo(eventId, todoId, user) {
    try {
      validatePermission(user, 'menghapus todo');
  ...
  /**
   * Add rundown item (requires permission)
   * @param {number|string} eventId
   * @param {Object} data - Rundown item data
   * @param {Object} user - Current user object
   * @returns {Promise<Object>}
   */
  static async addRundownItem(eventId, data, user) {
    try {
      validatePermission(user, 'menambah rundown');
  ...
  /**
   * Update rundown item (requires permission)
   * @param {number|string} eventId
   * @param {number|string} rundownId
   * @param {Object} data - Updated data
   * @param {Object} user - Current user object
   * @returns {Promise<Object>}
   */
  static async updateRundownItem(eventId, rundownId, data, user) {
    try {
      validatePermission(user, 'mengupdate rundown');
  ...
  /**
   * Delete rundown item (requires permission)
   * @param {number|string} eventId
   * @param {number|string} rundownId
   * @param {Object} user - Current user object
   * @returns {Promise<Object>}
   */
  static async deleteRundownItem(eventId, rundownId, user) {
    try {
      validatePermission(user, 'menghapus rundown');
  ...
  /**
   * Reorder rundown items (requires permission)
   * @param {number|string} eventId
   * @param {Array} orders - Array of {id, sort_order}
   * @param {Object} user - Current user object
   * @returns {Promise<Object>}
   */
  static async reorderRundown(eventId, orders, user) {
    try {
      validatePermission(user, 'mengatur ulang rundown');
  ...
  /**
   * Add budget item (requires permission)
   * @param {number|string} eventId
   * @param {Object} data - Budget item data
   * @param {Object} user - Current user object
   * @returns {Promise<Object>}
   */
  static async addBudgetItem(eventId, data, user) {
    try {
      validatePermission(user, 'menambah budget');
  ...
  /**
   * Update budget item (requires permission)
   * @param {number|string} eventId
   * @param {number|string} budgetId
   * @param {Object} data - Updated data
   * @param {Object} user - Current user object
   * @returns {Promise<Object>}
   */
  static async updateBudgetItem(eventId, budgetId, data, user) {
    try {
      validatePermission(user, 'mengupdate budget');
  ...
  /**
   * Delete budget item (requires permission)
   * @param {number|string} eventId
   * @param {number|string} budgetId
   * @param {Object} user - Current user object
   * @returns {Promise<Object>}
   */
  static async deleteBudgetItem(eventId, budgetId, user) {
    try {
      validatePermission(user, 'menghapus budget');
 ...
 * 
 * import { EventAPI, hasPermission, ALLOWED_ROLES } from '@/lib/api-client';
 * import { toast } from 'sonner';
 * 
 ...
 *   const canEdit = hasPermission(user, ALLOWED_ROLES.FULL_ACCESS);
 *   
 *   // Get all events (no permission needed)
 ...
 *     const result = await EventAPI.create(formData, user);
 *     
 *     if (result.success) {
 *       toast.success('Event berhasil dibuat');
 *       loadEvents();
 *     } else if (result.code === 403) {
 *       toast.error(result.error); // Permission error
 *     } else {
 ...
 *     const result = await EventAPI.update(id, formData, user);
 *     
 *     if (result.success) {
 ...
 *     const result = await EventAPI.delete(id, user);
 *     
 *     if (result.success) {
 ...
 *     <div>
 *       {canEdit && (
 *         <button onClick={() => handleCreate(newEventData)}>
 *           Tambah Event
 *         </button>
 *       )}
 *       
 ...
 *           {canEdit && (
 *             <>
 *               <button onClick={() => handleUpdate(event.id, updatedData)}>
 *                 Edit
 *               </button>
 ...
 * 
 * // COMMITTEE EXAMPLE:
 * const handleAddCommittee = async (eventId, memberData) => {
 *   const result = await EventAPI.addCommitteeMember(eventId, memberData, user);
 *   if (result.success) {
 *     toast.success('Anggota panitia berhasil ditambahkan');
 *   } else if (result.code === 403) {
 *     toast.error(result.error);
 *   }
 * };
 * 
 * // TODO EXAMPLE:
 * const handleUpdateTodoStatus = async (eventId, todoId, status) => {
 *   const result = await EventAPI.updateTodoStatus(eventId, todoId, status, user);
 *   if (result.success) {
 *     toast.success('Status todo berhasil diupdate');
 *   } else if (result.code === 403) {
 *     toast.error(result.error);
 *   }
 * };
 * 
 * // BUDGET EXAMPLE:
 * const handleAddBudget = async (eventId, budgetData) => {
 *   const result = await EventAPI.addBudgetItem(eventId, budgetData, user);
 *   if (result.success) {
 *     toast.success('Item budget berhasil ditambahkan');
 *   } else if (result.code === 403) {
 *     toast.error(result.error);
 *   }
 * };
